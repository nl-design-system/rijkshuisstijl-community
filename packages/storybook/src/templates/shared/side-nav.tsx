...
export const SharedSideNav = ({ currentHref }: { currentHref?: string }) => {
  ...
  return (
    <>
      ...
      <button className="rhc-shared-side-nav__trigger" onClick={() => setOpen(true)} type="button">
        <Icon icon="menu" />
        Menu
      </button>
      {open ? (
        ...
        >
          <header className="utrecht-drawer__header rhc-shared-side-nav__drawer-header" key="header">
            <button className="rhc-shared-side-nav__drawer-close" onClick={() => setOpen(false)} type="button">
              Sluiten
              <Icon icon="kruis" />
            </button>
          </header>
