...
# Design Tokens for the CJIB Design System
...
```shell
npm install --save --save-exact @rijkshuisstijl-community/cjib-design-tokens
```
...
Opt-in way to apply all the CSS variables to HTML elements with the `cjib-theme` class name:

```html
<!doctype html>
<html class="cjib-theme">
  <!-- var(--cjib-color-zwart) works here...! -->
</html>
```
...
This method should only be used in case there is no control over the HTML template and the `cjib-theme` class name cannot be included.
...
```js
import { cjibButtonFontSize } from '@rijkshuisstijl-community/cjib-design-tokens/dist/index.mjs';
```

### `dist/_variables.scss`

```scss
@use '~@rijkshuisstijl-community/cjib-design-tokens/dist/variables';
...
@media (min-width: $cjib-viewport-scale-xl-width) {
  --cjib-heading-1-font-size: 3rem;
}
```
...
$body-bg: $cjib-document-background-color;
$body-color: $cjib-document-color;
```
...
```html
<link rel="stylesheet" href="https://unpkg.com/@rijkshuisstijl-community/cjib-design-tokens/dist/theme.css" />
```

Then add the `cjib-theme` class name to your page:
...
```css
.my-button {
  background-color: var(--cjib-color-red-50);
  color: var(--cjib-color-white);
}
```
