...
- cb9bc2c: Added new tokens for section, grid and footer and changed page.size.md to account for padding-inline

  Section tokens:
  - rhc.page-section.padding-inline
    - ook voor viewport sm, md, lg onder -vi-small-, -vi-medium- en -vi-large-
  - rhc.page-section.max-inline-size - set to 90rem, to account for padding-inline.
  ...
  Footer tokens:
  - alle utrecht.page-footer.\* weggehaald in ruil voor:
  - rhc.page-footer.compact.tagline.font-size
  - rhc.page-footer.compact.tagline.color
  - rhc.page-footer.compact.tagline.background-color
  - rhc.page-footer.primary.padding-block
  - rhc.page-footer.primary.row-gap
  - rhc.page-footer.secondary.padding-block
  - rhc.page-footer.secondary.row-gap
  - rhc.page-footer.navigation-bar.column-gap
  - rhc.page-footer.navigation-bar.row-gap
  - rhc.page-footer.navigation-bar.link.text-decoration-line
  - rhc.page-footer.navigation-bar.link.min-block-size
  - rhc.page-footer.tagline.font-style
  - rhc.page-footer.tagline.font-size
  - rhc.page-footer.tagline.line-height
...
- 60699ab: Implemented multi-dimensional themes (theme groups)
- daa5c99: Remove `clamp()` font sizes from `rhc-theme`, make fluid font sizes opt-in.

### Minor Changes

- 5c1c404: - `common.border-radius.sm` aangepast van `2.5px` naar `2px`, `common.border-radius.md` van `5px` naar `4px`
  - Styling van form-field inputs aangepast op aankomende Rijkshuisstijl richtlijnen
- d977c11: - Component tokens toegevoegd voor `nl-button`.
  - Extra brand tokens toegevoegd voor `rhc.color.groen.*` en `rhc.color.rood.*` en common tokens voor `rhc.color.positive.*` en `rhc.color.negative.*`, voor het stylen van de hint buttons. Alleen de tokens toegevoegd die daadwerkelijk gebruikt worden, overige stappen worden bewust pas toegevoegd wanneer ze nodig zijn.
  - Common tokens hernoemd voor consistentie met de naamgeving van `rhc.color.core.*` en `rhc.color.primary.*` tokens. De oude tokens zijn deprecated en worden in een toekomstige release verwijderd:
    - `rhc.color.positive.default` → `rhc.color.positive.500`
    - `rhc.color.positive.subtle` → `rhc.color.positive.50`
    - `rhc.color.info.default` → `rhc.color.info.500`
    - `rhc.color.info.subtle` → `rhc.color.info.50`
    - `rhc.color.negative.default` → `rhc.color.negative.500`
    - `rhc.color.negative.subtle` → `rhc.color.negative.50`
    - `rhc.color.warning.default` → `rhc.color.warning.500`
    - `rhc.color.warning.subtle` → `rhc.color.warning.50`
...
- 2949170: Add class names to enable fluid design and dark mode:
  - `rhc-theme--fluid`: fluid design tokens for all the things
  - `rhc-theme--fluid-text`: fluid typography tokens
  - `rhc-theme--fluid-space`: fluid space tokens for all the things
  - `rhc-theme--dark-mode`: enable switching to dark mode

  The modifier class names are available for the other theme class names too.

- 102d333: Swap px for rem where sizing should scale with the rest of the tokens.
- 8ba49b2: - Styling van accordion aangepast op aankomende Rijkshuisstijl richtlijnen
  - Styling van Rijkslogo aangepast op aankomende Rijkshuisstijl richtlijnen

### Patch Changes

- c414601: Ensure package is published with provenance
- 02786d9: security patch
- bf5eddc: Just code quality refactorings
...
- 9161104: - Tokensets herverdeeld ter voorbereiding op multidimensionele thema's
...
- c2775bf: Add `rhc.color.light.*` tokens. They're currently a copy of `rhc.color.*` tokens, but in the future the `.rhc.color.*` tokens will automatically switch between light and dark mode, and these tokens will then exist as foundation for dark mode and for folks who need a fixed color palette.
